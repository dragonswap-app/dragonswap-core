...
const addLiquidityERC20 = async (
  ...
  const amountAMin = await convertToDecimals(tokenA, amountA.mul(97).div(100));
  const amountBMin = await convertToDecimals(tokenB, amountB.mul(97).div(100));
  const deadline = (await getLatestBlockTimestamp()) + 3600;
