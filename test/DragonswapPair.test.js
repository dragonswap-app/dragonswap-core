...
const { expandTo18Decimals, mineBlock, encodePrice, currentTimestamp } = require('./helpers.js');
const { solidity } = require('ethereum-waffle');
const { waffle } = require('hardhat');
const { loadFixture } = waffle;
const { pairFixture } = require('./fixtures');
...
describe('DragonswapPair', () => {
  let factory, token0, token1, pair, wallet, other;
  ...
  swapTestCases.forEach((swapTestCase, i) => {
    it(`getInputPrice:${i}`, async () => {
      ...
      ).to.be.revertedWith('Dragonswap: K');
  ...
  it('swap:gas', async () => {
    ...
    const allowedVariation = 10000;
    expect(receipt.gasUsed).to.be.within(
      107351 - allowedVariation,
      116696 + allowedVariation
    );
  });
  ...
  it('price{0,1}CumulativeLast', async () => {
    ...
    // await mineBlock(blockTimestamp + 1); // Block is auto-mined
    ...
    await token0.transfer(pair.address, swapAmount);
    await mineBlock(blockTimestamp + 9); // One block is auto-mined
    ...
    await mineBlock(blockTimestamp + 19); // One block is auto-mined
    await pair.sync(overrides);
